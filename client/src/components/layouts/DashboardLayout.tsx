...
export const DashboardLayout: React.FC<DashboardLayoutProps> = ({
  ...
  const { data, loading, error } = useMeQuery();
   ...
   console.log('user data', data?.me?.user_type) 
  if (!data?.me?.user_type && loading) {
    return <div>loading</div>;
  }
  if (data?.me?.user_type === undefined ) {
    console.log('error')
  }
    ...
    <>
      <MainLayout>
        <Flex mx="auto" w="80%" mt={50}>
          ...
          {data?.me?.user_type === "company" ? <Sidebar username={data?.me?.username}/> : <JsSidebar username={data?.me?.username} />}
