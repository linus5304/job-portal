...
const Job: React.FC<JobProps> & layout = ({}) => {
  ...
  const {data} = useGetJobByIdQuery({variables: {
    id: jobId
  }})
  ...
  return (
    <>
        ...
        <Flex mt={[0, 0, 0, "-10%", "-10%"]}>
              ...
              <Box px={["3%", "3%", "3%", "auto", "auto"]} >
                ...
                >
                    ...
                    <VStack spacing={6}>
                      <Image src={data?.getJobById.company.logo} alt="logo" width="100px" height="100px"/>
                      <Text fontSize="xl">About {data?.getJobById.company.name} Employer</Text>
      ...
      <Flex h="100%" py="2%" bg="#fff">
                <Flex w="60%" m="auto" justifyContent="space-between" flexDir={['column','column','column', 'row', 'row']}>
                
                    <Button size="lg" ml="-15%">APPLY NOW</Button>
                    
                    
                    <HStack spacing="24px">
                    <Text>Share this job</Text>
                    <Link ><Icon as={FaFacebook} fontSize="lg"/></Link>
        <Link ><Icon as={FaTwitter} fontSize="lg"/></Link>
        <Link ><Icon as={FaInstagram} fontSize="lg"/></Link>
        <Link ><Icon as={FaLinkedin} fontSize="lg"/></Link>
                    </HStack>
                </Flex>
